...
export default async function Page({ params }) {
	...
	const filtered = products.filter((p) => p.category === category && p.type === subcategory);
	return (
		<main className="min-h-screen bg-[#f8fafc] py-10">
			<div className="container mx-auto px-4">
				<h1 className="text-3xl font-bold mb-6 text-[#2d1a10] capitalize">{category} / {subcategory}</h1>
				<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
					{filtered.length === 0 ? (
						<div className="col-span-full text-center text-[#7c6a58]">No products found in this subcategory.</div>
					) : (
