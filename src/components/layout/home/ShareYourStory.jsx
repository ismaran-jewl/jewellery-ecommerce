...
import { SHARE_YOUR_STORY_STORIES as stories, SHARE_YOUR_STORY_SOCIALS as socialPosts } from "@/config/home";

/* ─────────────────────────────────────────────────── */
/* Floating polaroid — entrance only, gentle parallax */
/* ─────────────────────────────────────────────────── */
function FloatingImage({ story, mouseX, mouseY }) {
  // Reduced parallax range from 50px to 15px
  const xRange = [15 * (story.id % 2 === 0 ? 1 : -1), -15 * (story.id % 2 === 0 ? 1 : -1)];
  const yRange = [15 * (story.id % 2 === 0 ? -1 : 1), -15 * (story.id % 2 === 0 ? -1 : 1)];
  ...
  return (
    <motion.div
      ...
      className="absolute hidden lg:block w-52 aspect-[3/4] p-2 shadow-xl rounded-lg z-10 hover:z-30 transition-all duration-500 group cursor-pointer"
      whileHover={{ scale: 1.05, rotate: 0, transition: { duration: 0.3 } }}
    >
      {/* No constant bobbing animation — just entrance + parallax */}
...
/* ─────────────────────────────────────────────────── */
/* Social post card — entrance only, gentle parallax  */
/* ─────────────────────────────────────────────────── */
function SocialPostCard({ post, mouseX, mouseY }) {
  const Icon = post.icon;
  // Reduced parallax range from 30px to 10px
  const xRange = [10 * (post.id % 2 === 0 ? 1 : -1), -10 * (post.id % 2 === 0 ? 1 : -1)];
  const yRange = [10 * (post.id % 2 === 0 ? -1 : 1), -10 * (post.id % 2 === 0 ? -1 : 1)];

  const x = useTransform(mouseX, [0, 1], xRange);
  const y = useTransform(mouseY, [0, 1], yRange);
  const xSpring = useSpring(x, { stiffness: 25, damping: 30 });
  const ySpring = useSpring(y, { stiffness: 25, damping: 30 });
  ...
  return (
    <motion.div
      ...
      className="absolute hidden lg:block w-28 aspect-square p-1.5 shadow-lg rounded-lg z-0 hover:z-20 hover:opacity-100 transition-all duration-700 cursor-pointer grayscale hover:grayscale-0 group"
    >
      {/* No constant bobbing — just entrance + parallax */}
      <div className="w-full h-full relative bg-white rounded-md overflow-hidden p-1 border border-neutral-100">
...
export default function ShareYourStory() {
  ...
  return (
    ...
    >
      ...
      />
      <div
        className="absolute bottom-1/4 right-1/4 w-[500px] h-[500px] rounded-full blur-[150px] pointer-events-none z-[1]"
        style={{ background: "#F5F5F0", opacity: 0.1 }}
      />

      {/* Floating images (desktop) — entrance + parallax only */}
      ...
      <div className="relative z-20 max-w-4xl mx-auto px-6 text-center">
        ...
        >
          {/* Badge */}
          <div
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border text-xs tracking-[0.2em] uppercase mb-10 backdrop-blur-md"
            style={{
